...
- PyInstaller packaging (Phase 10 of original 10-phase plan — only phase not yet complete)
...
**`detectLanguage()` implementation details (verified from source):** Scans `(c.evidence || '') + ' ' + (c.finding || '')` for ALL controls — not just the current card. Counts file extension matches using `(?=[:,(\\s]|$)` lookahead to avoid mid-word false positives. Returns the most frequent language across all extensions. `.kt` maps to `'Java'`. Returns `null` if no extensions found. The `_LANG_KEYS` list that `filterReviewSteps()` uses as section headers is `['Python','Js/Ts','Rust','Java','C/C++','C#','Go','Php']` — these exact strings must appear as line prefixes in the review procedure text. Lines before the first language-keyed section are shown for all languages ("general steps"). If filtering produces an empty string, falls back to the full text.
...
5 of 12 auto, 2 manual, 5 review_required (no scanner). **`os-software-controls.md` header bug:** the file header says "12 controls across 5 families" but there are actually 6 families (PATCH, EOL, SOFTINV, SVCCONFIG, SVCEXPOSE, OSAUDIT — the OSAUDIT family is present in the summary table but missed in the header count).
...
**`SEC-AUTH-001` Python pattern is effectively broken.** The regex `r'@app\.route.*\ndef\s+\w+.*\n(?:(?!login_required|...).)*$'` contains `\n` newline sequences and expects multi-line content, but `scan_file()` applies it to one line at a time via `re.search(pattern, line, re.IGNORECASE)`. The regex will never match a single line and never fires. SEC-AUTH-001 effectively has no code coverage for Python.
...
**`ScanResult.elapsed_seconds` is a dead field.** The dataclass includes `elapsed_seconds: float = 0.0` (line 41). Several scanners in `scanners.py` populate it (e.g., TLSScanner sets `elapsed_seconds=time.time() - start`). However, `engine.py` never reads `sr.elapsed_seconds` — it is not copied to `AssessmentResult`, not stored in `FindingsDB`, and not serialized in any report output. It exists as instrumentation scaffolding that was never wired up.

**`HAS_BS4` (BeautifulSoup) is a dead import in `scanners.py`.** It is imported at lines 27–30 with a try/except guard, but `HAS_BS4` and `BeautifulSoup` are never referenced anywhere else in the file. No scanner conditionally uses BS4. This is a leftover from an earlier iteration — BeautifulSoup4 is not required for any current scanner functionality. Note: `beautifulsoup4>=4.12.0` IS listed in `requirements.txt`, so it will be installed in any venv, but that makes no behavioral difference since the code never calls it.
...
**`fix_text` is ONLY populated for STIG controls.** `_parse_control_section()` (regular control parser, lines 246–260) does NOT include `fix_text` in the `Control()` constructor — it is left at the dataclass default `""`. Even if a control library `.md` has a `**Fix**:` or `**Fix Text**:` field, the content is recognized as `_known` (preventing `review_procedure` leakage) but NOT extracted to `ctrl.fix_text`. Only `parse_stig_controls()` (line 308) sets `fix_text=fields.get('fix', '')`. Consequence: for the engine's remediation fallback (`if not ar.remediation: ar.remediation = ar.control.fix_text or ar.control.statement`), non-STIG controls always fall through to `ar.control.statement`. STIG controls use the XCCDF `fix` field.
...
git commit -m "Rounds 13-23 handoff — Round 23: STIG controls hardcode review_required and skip classify_control(); STIG scan falls to else-branch running WEBSITE_SCANNERS against XML path (ERROR results, harmless); line 980 corrected (was 'zero scanner results'); manual loop always emits statement (not fallback when test_procedure empty); manual-tier confidence stays 1.0 (never set); ScanResult.elapsed_seconds is dead field; Round 22 and prior corrections retained"
git push
```
